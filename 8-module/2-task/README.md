...
export default class CartIcon {
  ...
  // Заполнить её данными из объекта cart (детали описаны ниже)
  update(cart) {
    if (!cart.isEmpty()) {
...
// добавляем данные из объекта корзины
...
Иконка корзины отвечает за отображение количества товаров и их общей стоимости.
Данные о товарах находятся в объекте корзины `cart`.
...
Подробнее про `position: fixed` - [тут](https://learn.javascript.ru/position#position-fixed).
