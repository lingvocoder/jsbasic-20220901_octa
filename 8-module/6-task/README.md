...
В этой задаче у вас уже есть файл `CartIcon.js`, в котором находится класс `Cart`, описывающий этот компонент. Перед началом работы, скопируйте реализацию методов `addProduct`, `updateProductCount`, `isEmpty`, `getTotalCount`, `getTotalPrice` в класс `Cart` из предыдущей задачи.
